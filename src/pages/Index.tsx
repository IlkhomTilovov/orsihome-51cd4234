        ...
        <>
          {items.map((p, i) => (
            <div key={(p?.id || 'e') + '-' + i} className="h-full">
              <ProductCard product={p} imageAspect="aspect-[16/10]" />
            </div>
          ))}
                    ...
                    <div className="absolute inset-0">
                      {img ? (
                        <img
                          ...
                          width={400}
                          height={500}
                          className="w-full h-full object-contain bg-card group-hover:scale-105 transition-transform duration-700 ease-luxe"
                        />
                      ) : FallbackIcon ? (
